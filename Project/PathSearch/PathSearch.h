...
namespace AStarPathfinding { namespace algorithms {
	...
	private:
		...
		struct PlannerNode
		{
			...
			void AddValue(SearchNode* _search, PlannerNode* _parent)
			{
				searchNode = _search;
				parent = _parent;
			}
			PlannerNode(SearchNode* _search)
			{
				searchNode = _search;
				parent = nullptr;
			}
		...
		TileMap* tileMap;
		Tile* end;
		PlannerNode* goal = new PlannerNode();
		bool done = false;
		float hW = 1.3;
		float TD;
		//PlannerNode* start = new PlannerNode();
		//fullsail_ai::fundamentals::QueueList<PlannerNode*> open;
		PriorityQueue<PlannerNode*, Comp> open;
	...
	public:
		...
		bool isGreater(PlannerNode* lhs, PlannerNode* rhs);
